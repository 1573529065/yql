...
class SolarTerm
{
    public static function getSolarTerms($year = '', $timezone_or_longitude = 'Asia/Shanghai')
    {
        self::setTimezone($timezone_or_longitude);
        ...
        $resultArr = [];
        for ($i = 0; $i < 25; $i++) {
            $julianDay = self::CalculateSolarTerms($year, $start * 15);
            $resultArr[$i] = [self::JDTDtoLocalTime($julianDay), 'term_code' => $start];
        ...
        foreach ($resultArr as $k => &$v) {
            $v['time'] = self::JDaysToDatetime($v[0])->format('Y-m-d');
            $v['term_name'] = self::getSolorTermName($v['term_code']);
        }
     ...
     * @param string $timezone_or_longitude
     * @return mixed|null
     * @throws \Exception
     */
    public static function getPrevSolarTerm($datetime = '', $timezone_or_longitude = 'Asia/Shanghai')
    {
        if (!($datetime instanceof \DateTime) && !is_string($datetime)) throw new \Exception('only could accept DateTime object or string date(yyyy-m-d h-i-s)');

        //Set timezone or longitude!
        if (is_numeric($timezone_or_longitude)) {
            throw new \Exception('write out the part of accept longitude!! it unfinished here');
            //write longitude to function this class that is JDTDtoLocalTime();
        } else
            if ($timezone_or_longitude) {
                self::setTimezone($timezone_or_longitude);
            }
        ...
        $solarTerms = self::getSolarTerms($datetime->format('Y'));
        $result = null;
        foreach ($solarTerms as $v) {
            if ($v[0]->format('U') > $datetime->format('U')) continue;
            if (!$result) {
                $result = $v;
                continue;
            }
            if ($v[0]->format('U') > $result[0]->format('U'))
                $result = $v;
        }
        return $result;
    }

    /** Set the time zone which affecting every result after then.
     * numeric timezone should be receivable but need to finish!
     *
     * @param $value
     *  a location name string according to php.net manual/ numeric timezone should be receivable but need to finish!
     * @throws \Exception   until numeric timezone code is done the Exception would be here when you input a integer.
     */
    public static function setTimezone($value)
    {
        if (is_numeric($value)) throw new \Exception('here is not finished that need the logic of function of numeric timezone!');
        date_default_timezone_set($value);
        self::$timezone_or_longitude = $value;
    }

    private static $timezone_or_longitude = 'Asia/Shanghai';
    ...
    private static $SOLAR_TERMS_COUNT = 24;
    // 节气定义
    const  TERMS_VERNAL_EQUINOX = 0;    // 春分
    const  TERMS_CLEAR_AND_BRIGHT = 1;    // 清明
    const  TERMS_GRAIN_RAIN = 2;    // 谷雨
    const  TERMS_SUMMER_BEGINS = 3;    // 立夏
    const  TERMS_GRAIN_BUDS = 4;    // 小满
    const  TERMS_GRAIN_IN_EAR = 5;    // 芒种
    const  TERMS_SUMMER_SOLSTICE = 6;    // 夏至
    const  TERMS_SLIGHT_HEAT = 7;    // 小暑
    const  TERMS_GREAT_HEAT = 8;    // 大暑
    const  TERMS_AUTUMN_BEGINS = 9;    // 立秋
    const  TERMS_STOPPING_THE_HEAT = 10;   // 处暑
    const  TERMS_WHITE_DEWS = 11;   // 白露
    const  TERMS_AUTUMN_EQUINOX = 12;   // 秋分
    const  TERMS_COLD_DEWS = 13;   // 寒露
    const  TERMS_HOAR_FROST_FALLS = 14;   // 霜降
    const  TERMS_WINTER_BEGINS = 15;   // 立冬
    const  TERMS_LIGHT_SNOW = 16;   // 小雪
    const  TERMS_HEAVY_SNOW = 17;   // 大雪
    const  TERMS_WINTER_SOLSTICE = 18;   // 冬至
    const  TERMS_SLIGHT_COLD = 19;   // 小寒
    const  TERMS_GREAT_COLD = 20;   // 大寒
    const  TERMS_SPRING_BEGINS = 21;   // 立春
    const  TERMS_THE_RAINS = 22;   // 雨水
    const  TERMS_INSECTS_AWAKEN = 23;   // 惊蛰


    private static $JD2000 = 2451545.0;
    // 圆周率
    const PI = 3.1415926535897932384626433832795;
    // 一度代表的弧度
    private static $RADIAN_PER_ANGLE = 0.017453292519943;//self::PI / 180.0
    ...
    */
    ...
    private static function CalcEarthLongitudeNutation($dt)
    {
        ...
        for ($i = 0; $i < sizeof(self::$NUTATION) / sizeof(self::$NUTATION[0]); $i++) {
            $sita = self::$NUTATION[$i][0] * $D + self::$NUTATION[$i][1] * $M + self::$NUTATION[$i][2] * $Mp +
                self::$NUTATION[$i][3] * $F + self::$NUTATION[$i][4] * $Omega;
    ...
    private static function TdUtcDeltatT($y)
    {
        ...
        if ($y >= 2005) {
            ...
            $t1 = (double)($y - self::$DELTA_TB1[$i][0]) / (double)(self::$DELTA_TB1[$i + 1][0] -
                    self::$DELTA_TB1[$i][0]) * 10.0;
            $t2 = $t1 * $t1;
            $t3 = $t2 * $t1;

            return self::$DELTA_TB1[$i][1] + self::$DELTA_TB1[$i][2] * $t1 + self::$DELTA_TB1[$i][3] *
                $t2 + self::$DELTA_TB1[$i][4] * $t3;
        }
    }
    ...
    private static function CalculateSolarTerms($year, $angle)
    {
        ...
        do {
            ...
            */
            ...
            $stDegreep = (self::GetSunEclipticLongitudeEC($JD0 + 0.000005)
                    - self::GetSunEclipticLongitudeEC($JD0 - 0.000005)) / 0.00001;
