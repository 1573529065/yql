 ...
 * Time: 4:12 PM
 */
namespace App\Vendor\Wechat;
use App\Vendor\Swoole\Http;
...
class Wechat
{
    ...
    {
        $this->token = isset($options['token'])?$options['token']:env('WECHAT_TOKEN');
        $this->encodingAesKey = isset($options['encodingaeskey'])?$options['encodingaeskey']:env('WECHAT_AES_KEY');
        $this->appid = isset($options['appid'])?$options['appid']:env('WECHAT_APPID');
        $this->appsecret = isset($options['appsecret'])?$options['appsecret']:env('WECHAT_SECRET');
        $this->debug = isset($options['debug'])?$options['debug']:false;
        $this->logcallback = isset($options['logcallback'])?$options['logcallback']:function($log){
            Log::info(__CLASS__.":",[$log]);
        };
    }


    public function res($result,$key=null,$hasKey=true){
        if ($result) {
            if(is_string($result)){
            ...
            if(!empty($key)&&isset($json[$key])) return $json[$key];
    ...
    protected function setCache($cachename,$value,$expired){
        ...
        $redis->set("laravel:".$cachename,$value);
        $redis->expire("laravel:".$cachename,$expired);
    ...
    protected function getCache($cachename){
        ...
        $redis=Redis::connection("default");
        return $redis->get("laravel:".$cachename);
    }
    ...
    protected function removeCache($cachename){
        ...
        $redis=Redis::connection("default");
        return $redis->del(["laravel:".$cachename]);
    }
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @return bool|mixed
     */
    private function http_get($url,$call=null){

        if($this->isSwoole){
            if(empty($call)) $call=$this->responseCallable;
            Http::get($url,[],$call);
        }else{
            $oCurl = curl_init();
    ...
    private function http_post($url,$param,$post_file=false){
        ...
        if($this->isSwoole){
            Http::post($url,$param,[],$this->responseCallable);
        }else{
            $oCurl = curl_init();
            if(stripos($url,"https://")!==FALSE){
                curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
    ...
    public function checkAuth($appid='',$appsecret='',$token=''){
        ...
        $self=$this;
        $result = $this->http_get(self::API_URL_PREFIX.self::AUTH_URL.'appid='.$appid.'&secret='.$appsecret,function ($result)use($self,$authname){
        ...
        if ($result)
        {
            return $self->res($result,"access_token");
        }
    ...
    public function getServerIp(){
        ...
        return $this->res($result,"ip_list");
    }
    ...
    public function getShortUrl($long_url){
        ...
        return $this->res($result,'short_url');
    ...
    public function getUserGroup($openid){
        ...
        return $this->res($result,"groupid",false);
    ...
    public function addTemplateMessage($tpl_id){
        ...
        return $this->res($result,"template_id",false);
    ...
    public function response(callable  $param=null)
    {
        $this->responseCallable=$param;
    }
}
/**
 * PKCS7Encoder class
