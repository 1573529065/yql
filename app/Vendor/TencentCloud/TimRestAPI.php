...
class TimRestAPI extends TimRestInterface
{
     ...
     */
    function init($identifier = '', $sdkappid = 0)
    {
        empty($sdkappid) ?: $this->sdkappid = $sdkappid;
        empty($identifier) ?: $this->identifier = $identifier;
        $privateKeyPath = $this->getPrivateKeyPath();
        $this->sdkappid = config('tencent-im.sdkAppId');
        $this->private_key_path = base_path($privateKeyPath);
        $this->signature = base_path('/app/Vendor/TencentCloud/signature/linux-signature64');
        $this->generate_user_sig($this->identifier, '', $this->private_key_path, $this->signature);
    }

    public function generateSig($userId)
    {
        return $this->generate_user_sig($userId, '', $this->private_key_path, $this->signature)[0];
    }

    protected function getPrivateKeyPath()
    {
        $privateKeyPath = '/app/Vendor/TencentCloud/keys/%s/private_key';
        ...
        return sprintf($privateKeyPath, $path);

    }

    /**
     * TimRestAPI constructor.
     * @param string $identifier
     * @param int $sdkappid
     */
    function __construct($identifier = '', $sdkappid = 0)
    {
        $this->init($identifier, $sdkappid);
    }
...
//        }
    ...
    public function multiaccount_import($identifiers)
    {
        $msg = ['Accounts' => $identifiers];

        $reqData = json_encode($msg);

        $ret = $this->api("im_open_login_svc", "multiaccount_import", $this->identifier, $this->usersig, $reqData);
        $ret = json_decode($ret, true);
        return $ret;
    }
    ...
    public function sns_friend_batch_import($account_id, $receivers)
    {
        ...
        foreach ($receivers as $receiver) {
            $receiver_arr[] = [
                'To_Account' => $receiver,
                'Remark' => "",
                'AddSource' => "AddSource_Type_Unknown",
                'AddWording' => ""
            ];
        }
    ...
    function group_create_group2($group_type, $group_name, $owner_id, $info_set, $mem_list)
    {
        ...
        $msg = array(
            ...
            'Introduction' => $info_set['introduction'] ?? null,
            'Notification' => $info_set['notification'] ?? null,
            'FaceUrl' => $info_set['face_url'] ?? null,
            'MaxMemberCount' => $info_set['max_member_num'] ?? null,
    ...
    function group_batch_delete_group_member(string $groupId, array $members, int $silence)
    {
        ...
        foreach ($members as $member) {
            array_push($memberList, (string)$member);
        }
        ...
        Log::debug('IM Debug', $memberList);

        #将消息序列化为json串
        $requestData = json_encode($msg);

        $ret = $this->api("group_open_http_svc", "delete_group_member", $this->identifier, $this->usersig, $requestData);
        $ret = json_decode($ret, true);
        return $ret;
    }
    ...
    function group_batch_add_group_member(string $groupId, array $members, int $silence)
    {
        ...
        foreach ($members as $member) {
            ...
            array_push($memberList, $memberItem);
        }
    ...
    public function uuid_url($file_type, $file_id, $storage_platform = 0)
    {
        ...
        #将消息序列化为json串
        $req_data = json_encode($msg);
        $ret = $this->api("rich_media", "query_file_url", $this->identifier, $this->usersig, $req_data);
        $ret = json_decode($ret, true);
        return $ret;
    }
}
...
function json_format($data, $indent = null)
{
    ...
    array_walk_recursive($data, __NAMESPACE__ . "\json_format_protect");
